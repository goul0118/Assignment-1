...
Ball::Ball()
{
    ...
    m_Texture = new OpenGLTexture("Ball");
...
void Ball::setballLives(int* _ballLives)
{
    ballLives = _ballLives;
...
void Ball::setBrickCounter(int* _brickCounter)
{
    brickCounter = _brickCounter;
...
void Ball::reset()
{
  ...
  setY((screenHeight - getRadius()) / 1.2f);
...
void Ball::checkCollision(GameObject* aGameObject)
{
  ...
  if(getIsActive() == true && aGameObject != NULL && aGameObject->getIsActive() == true)
  {
	...
	//Determine if the gameObject is a Block
...
void Ball::handleBrickCollision(Brick* _brick)
{
    //Calculate the ball's distance from the bricks
    float distanceX = fabsf(getX() - _brick->getX() - (_brick->getWidth() / 2.0f));
    float distanceY = fabsf(getY() - _brick->getY() - (_brick->getHeight() / 2.0f));

    //If the distance on the x-axis is greater than half-the-width of the brick + the ball's radius, then 
    //there is no way they can be colliding and return out of this method, no more collision handling is needed.
    if(distanceX > ((_brick->getWidth() / 2.0f) + getRadius()))
    {
      return; 
    }
    ...
    //there is no way they can be colliding and return out of this method, no more collision handling is needed.
    if(distanceY > ((_brick->getHeight() / 2.0f) + getRadius())) 
    {
      return; 
    }
    ...
    //if (getDirectionX() > 1)    //reset horizontal speed scalar
    //{
    //    setDirectionX(1);
    //}
    ...
    //the brick, set the ball's Y value and y-direction accordingly.
    if(distanceX <= (_brick->getWidth() / 2.0f))
    {
        _brick->setIsActive(false);
    ...
    if(distanceY <= (_brick->getHeight() / 2.0f))
    {
        _brick->setIsActive(false);
